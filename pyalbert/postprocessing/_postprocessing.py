...
def check_url(
            ...
            for protocol in protocols:
                if "/" in urlparse(url).path:  # If the url is like: example.../...
                    if urlparse(f"http://{url}").netloc.endswith(".gouv.fr"):
                        ...
                        url_out = f"{protocol}://{url}"  # example../.. becomes http(s)://example.../..
                    ...
                    if urlparse(f"http://{url}").netloc.endswith(".gouv.fr"):
                        ...
                        urlparse(f"{protocol}://www.{url}").netloc
                        == urlparse(good_url).netloc
                    ):
                        url_out = url

                    elif (
                        urlparse(f"{protocol}://{url}").netloc
                        == urlparse(good_url).netloc
                    ):
                        url_out = url
    ...
    if url.startswith("https://") or url.startswith("http://"):
        if urlparse(url).netloc.startswith(
            "www"
        ):  # If the url is like : http(s)://www.example...
            for good_url in whitelist:
                for protocol in protocols:
                    if (
                        ...
                        url_out = (
                            protocol
                            + "://"
                            + url.partition(f"{urlparse(url).scheme}://")[2]
                        )
...
def correct_url(
    ...
    def replace_url(match):
        ...
        return check_url(url=url, whitelist_path=whitelist_path, check_status_code=check_status_code)
...
def check_number(
    number: str, whitelist_path: str = "/data/whitelist/whitelist.json"
) -> str:
...
def correct_number(
    ...
    def replace_number(match):
        if (
            len(match.group(0)) == 4
            and int(match.group(0)) >= 1900
            and int(match.group(0)) < 2100
        ...
        num_dict["new_number"] = check_number(
            number=number, whitelist_path=whitelist_path
        )
...
def check_mail(
    mail: str, whitelist_path: str = "/data/whitelist/whitelist.json"
) -> str:
...
def correct_mail(
    text: str, whitelist_path: str = "/data/whitelist/whitelist.json"
) -> str:
